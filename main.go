...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, "  media-registry <subcommand>   Track metadata for removable media (SD cards, external drives)\n")
...
// =============================================================================
// Media Registry (track removable media metadata)
// =============================================================================

func mediaRegistryFile() string {
	return filepath.Join(defaultManifestRoot(), "media-registry.txt")
}

func runMediaRegistry(args []string) {
	if len(args) == 0 {
		printMediaRegistryUsage()
		return
	}
	...
	case "list":
		listMediaRegistry()
	case "add":
		addMediaRegistry(args[1:])
	case "update":
		updateMediaRegistry(args[1:])
	case "help", "--help", "-h":
		printMediaRegistryUsage()
	default:
...
func printMediaRegistryUsage() {
	fmt.Fprintf(os.Stderr, "Usage: photo-organizer media-registry <subcommand>\n\n")
	fmt.Fprintf(os.Stderr, "Manage removable media metadata registry\n\n")
	fmt.Fprintf(os.Stderr, "Subcommands:\n")
	fmt.Fprintf(os.Stderr, "  list                     Show all registered media\n")
	fmt.Fprintf(os.Stderr, "  add <id> <description>   Register new media\n")
	fmt.Fprintf(os.Stderr, "  update <id> <field> <value>  Update media metadata\n\n")
	fmt.Fprintf(os.Stderr, "Examples:\n")
	fmt.Fprintf(os.Stderr, "  photo-organizer media-registry list\n")
	fmt.Fprintf(os.Stderr, "  photo-organizer media-registry add \"sony-a7iv-card-1\" \"SanDisk Pro 64GB\"\n")
	fmt.Fprintf(os.Stderr, "  photo-organizer media-registry update \"sony-a7iv-card-1\" location \"camera bag\"\n")
}

func listMediaRegistry() {
	regFile := mediaRegistryFile()
	content, err := os.ReadFile(regFile)
	if err != nil {
		fmt.Fprintf(os.Stderr, "No media registry found. Create one with:\n")
		fmt.Fprintf(os.Stderr, "  photo-organizer media-registry add <id> <description>\n")
		return
	}

	fmt.Printf("\n═══════════════════════════════════════════════════════════\n")
	fmt.Printf("Registered Removable Media\n")
	fmt.Printf("═══════════════════════════════════════════════════════════\n\n")
	...
	for _, line := range lines {
		...
		if len(parts) > 2 {
			capacity = strings.TrimSpace(parts[2])
		}
		if len(parts) > 3 {
			lastScanned = strings.TrimSpace(parts[3])
		}
		if len(parts) > 4 {
	...
	fmt.Printf("To register new media:\n")
	fmt.Printf("  photo-organizer media-registry add \"<id>\" \"<description>\"\n\n")
	fmt.Printf("To use media during scan:\n")
	fmt.Printf("  photo-organizer scan /path --media-id \"<id>\"\n")
}

func addMediaRegistry(args []string) {
	if len(args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: photo-organizer media-registry add <id> <description>\n")
		fmt.Fprintf(os.Stderr, "Example: photo-organizer media-registry add \"sony-a7iv-card-1\" \"SanDisk 64GB\"\n")
		os.Exit(1)
	}
	...
	for _, line := range lines {
		...
		if len(parts) > 0 && strings.TrimSpace(parts[0]) == id {
			fmt.Fprintf(os.Stderr, "Error: media-id '%s' already exists\n", id)
			os.Exit(1)
		}
	}
	...
	entry := fmt.Sprintf("%s | %s | | %s | | active\n", id, description, now)

	// Ensure directory exists
	os.MkdirAll(filepath.Dir(regFile), 0755)

	// Append entry
	if err := os.WriteFile(regFile, append(content, []byte(entry)...), 0644); err != nil {
		fmt.Fprintf(os.Stderr, "Error writing registry: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("✓ Registered media: %s\n", id)
	fmt.Printf("Use with: photo-organizer scan /path --media-id \"%s\"\n", id)
}

func updateMediaRegistry(args []string) {
	if len(args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: photo-organizer media-registry update <id> <field> <value>\n")
		fmt.Fprintf(os.Stderr, "Fields: capacity, location, status, notes\n")
		fmt.Fprintf(os.Stderr, "Example: photo-organizer media-registry update \"sony-a7iv-card-1\" location \"camera bag\"\n")
		os.Exit(1)
	}
	...
	content, err := os.ReadFile(regFile)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: media registry not found\n")
		os.Exit(1)
	}
	...
	for _, line := range lines {
		...
		if len(parts) > 0 && strings.TrimSpace(parts[0]) == id {
			found = true
			// Rebuild entry with updated field
			id := parts[0]
			description := parts[1]
			capacity := ""
			lastScanned := ""
			...
			newLine := fmt.Sprintf("%s|%s|%s|%s|%s|%s|%s", id, description, capacity, lastScanned, location, status, notes)
	...
	if !found {
		fmt.Fprintf(os.Stderr, "Error: media-id '%s' not found\n", id)
		os.Exit(1)
	}

	// Write back
	if err := os.WriteFile(regFile, []byte(strings.Join(output, "\n")), 0644); err != nil {
		fmt.Fprintf(os.Stderr, "Error writing registry: %v\n", err)
		os.Exit(1)
	}
