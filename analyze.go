...
func runMigrate(args []string) {
	...
	fmt.Fprintf(out, "#\n")
	fmt.Fprintf(out, "# Files are copied preserving their folder structure under the source\n")
	fmt.Fprintf(out, "# scan root. Each rsync block handles one source scan path.\n")
	fmt.Fprintf(out, "# Review and run: bash migrate.sh\n")
	fmt.Fprintf(out, "#\n")
	...
	for _, scanPath := range scanPaths {
		...
		// Destination mirrors the source scan root name under dest root.
		...
		fmt.Fprintf(out, "# Destination: %s\n", destPath)
		fmt.Fprintf(out, "rsync -av --mkpath --files-from=- %s %s << 'RSYNC_FILES'\n",
			shellQuote(scanPath+"/"), shellQuote(destPath+"/"))
		for _, row := range rows {
			fmt.Fprintf(out, "%s\n", filepath.ToSlash(row.RelativePath))
		}
		fmt.Fprintf(out, "RSYNC_FILES\n\n")
	}
